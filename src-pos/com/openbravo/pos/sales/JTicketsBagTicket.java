...
//        m_jEdit.setVisible(m_App.getAppUserView().getUser().hasPermission("sales.EditTicket"));
        ...
        m_jPrint.setVisible(m_App.getAppUserView().getUser().hasPermission("sales.PrintTicket"));
              m_jPrint.setVisible(false);
        // postcondicion es que  m_jPrint.setVisible(false); ticket activado aqui y ticket en el panel
    }
  ...
  public void getDiscountDetails(TicketLineInfo oLine){
    ...
    private void m_jRefundActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_m_jRefundActionPerformed
        ...
        for(int i = 0; i < m_ticket.getLinesCount(); i++) {
            newline = new TicketLineInfo(m_ticket.getLine(i));
        ...
        refundticket.setShowName(m_ticket.getShowName());
        refundticket.setadvanceDate(m_ticket.getadvanceDate());
        refundticket.setRefundId(m_ticket.getTicketId());
        System.out.println("enter----"+m_ticket.getTicketId());
        refundticket.setTicketDateDate(m_ticket.getDate());
