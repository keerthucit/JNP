...
public class DataLogicReceipts extends BeanFactoryDataSingle {
    ...
    public final List<SeatInfo> getSeatList(String showId) throws BasicException {
        
        return (List<SeatInfo>) new StaticSentence(s
                , "SELECT SEATID, STATUS FROM SHOWLINES WHERE SHOWID='" + showId + "'  AND DATE=(SELECT CURDATE() FROM DUAL)  ORDER BY SHOWID"
                , null
                , new SerializerReadClass(SeatInfo.class)).list();
    }
    ...
    public final void insertShowLines(final String id, final TicketInfo ticket) throws BasicException {
        
        Object[] values = new Object[] {id, ticket.getName(), ticket};
        Datas[] datas = new Datas[] {Datas.STRING, Datas.STRING, Datas.SERIALIZABLE};
        
        new PreparedSentence(s
            , "INSERT INTO SHOWLINES (ID, NAME,CONTENT) VALUES (?, ?, ?)"
            , new SerializerWriteBasicExt(datas, new int[] {0, 1, 2})).exec(values);
    }
}
