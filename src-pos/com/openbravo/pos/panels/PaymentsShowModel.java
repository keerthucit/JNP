 ...
 * @author adrianromero
 */
public class PaymentsShowModel {
    ...
    private static String sysDate;
     private static String advDate;
    private String m_StartTime;
    private String m_EndTime;
    private static String m_ShowName;
    private static String sysTime;
////    private java.util.List<ProductsLine> m_lProducts;
    
    private final static String[] SALEHEADERS = {"label.taxcash", "label.totalcash"};

    private PaymentsShowModel() {
        sysDate = format.format(createdDate);
        sysTime = dt.format(createdDate);
        advDate = df.format(createdDate);
    }    
    
    public static PaymentsShowModel emptyInstance() {
        
        PaymentsShowModel p = new PaymentsShowModel();
        ...
        p.m_EndTime = null;
        p.m_ShowName = null;
//        p.m_lProducts = new ArrayList<ProductsLine>();
        
        return p;
    }
    
    public static PaymentsShowModel loadInstance(AppView app, String showName) throws BasicException {
        
        PaymentsShowModel p = new PaymentsShowModel();
        
        // Propiedades globales
        p.m_sHost = app.getProperties().getHost();
        p.m_iSeq = app.getActiveShowSequence();
        p.m_dDateStart = app.getActiveShowDateStart();
        p.m_dDateEnd = app.getActiveShowDateEnd();
        
         p.m_StartTime = null;
        p.m_EndTime = null;
         p.m_ShowName = showName;
           p.m_dActive = "Y";
        // Pagos
        Object[] valtickets = (Object []) new StaticSentence(app.getSession()
            , "SELECT COUNT(*), COALESCE(SUM(PAYMENTS.TOTAL),0) " +
              "FROM PAYMENTS, RECEIPTS " +
              "WHERE PAYMENTS.RECEIPT = RECEIPTS.ID AND RECEIPTS.MONEYSHOW = ? AND PAYMENTS.PAYMENT='cash'"
            , SerializerWriteString.INSTANCE
            , new SerializerReadBasic(new Datas[] {Datas.INT, Datas.DOUBLE}))
            .find(app.getActiveShowIndex());
        ...
        Object[] valReftickets = (Object []) new StaticSentence(app.getSession()
            , "SELECT SUM(TICKETLINES.PRICE) " +
               "FROM RECEIPTS, TICKETLINES, PRODUCTS, TICKETS WHERE RECEIPTS.ID = TICKETS.ID AND TICKETS.ID = TICKETLINES.TICKET AND PRODUCTS.ID= TICKETLINES.PRODUCT " +
                "AND TICKETS.TICKETTYPE=1 AND RECEIPTS.MONEYSHOW = ? " 
        ...
        Object[] valCancelFee = (Object []) new StaticSentence(app.getSession()
            , "SELECT COALESCE(SUM(TICKETS.CANCELLATIONFEE),0) " +
              ...
              "WHERE TICKETS.ID = RECEIPTS.ID AND RECEIPTS.MONEYSHOW = ? "
            , SerializerWriteString.INSTANCE
            , new SerializerReadBasic(new Datas[] {Datas.DOUBLE}))
            .find(app.getActiveShowIndex());
        ...
        List l = new StaticSentence(app.getSession()            
            , "SELECT PAYMENTS.PAYMENT, SUM(PAYMENTS.TOTAL) " +
              "FROM PAYMENTS, RECEIPTS " +
              "WHERE PAYMENTS.RECEIPT = RECEIPTS.ID AND RECEIPTS.ADVANCEBOOKING = 'N' AND RECEIPTS.MONEYSHOW = ? " +
              "GROUP BY PAYMENTS.PAYMENT"
            , SerializerWriteString.INSTANCE
            , new SerializerReadClass(PaymentsShowModel.PaymentsLine.class)) //new SerializerReadBasic(new Datas[] {Datas.STRING, Datas.DOUBLE}))
            .list(app.getActiveShowIndex());
        ...
        Object[] recsales = (Object []) new StaticSentence(app.getSession(),
            ...
            "FROM RECEIPTS, TICKETLINES WHERE RECEIPTS.ID = TICKETLINES.TICKET AND RECEIPTS.ADVANCEBOOKING = 'N' AND RECEIPTS.MONEYSHOW = ?",
            SerializerWriteString.INSTANCE,
            new SerializerReadBasic(new Datas[] {Datas.INT, Datas.DOUBLE}))
            .find(app.getActiveShowIndex());
        if (recsales == null) {
        ...
        Object[] showDetails = (Object []) new StaticSentence(app.getSession(),
        ...
        if (showDetails == null) {
            ...
            p.m_StartTime = (String) showDetails[0];
            p.m_EndTime = (String) showDetails[1];
        }
        ...
        Object[] rectaxes = (Object []) new StaticSentence(app.getSession(),
            ...
            "FROM RECEIPTS, TAXLINES WHERE RECEIPTS.ID = TAXLINES.RECEIPT AND RECEIPTS.ADVANCEBOOKING = 'N' AND RECEIPTS.MONEYSHOW = ?"
            , SerializerWriteString.INSTANCE
            , new SerializerReadBasic(new Datas[] {Datas.DOUBLE}))
            .find(app.getActiveShowIndex());
        if (rectaxes == null) {
    ...
    List<SalesLine> asales = new StaticSentence(app.getSession(),
                ...
                "AND RECEIPTS.ADVANCEBOOKING = 'N' AND RECEIPTS.MONEYSHOW = ? " +
                "GROUP BY PRODUCTS.NAME"
                , SerializerWriteString.INSTANCE
                , new SerializerReadClass(PaymentsShowModel.SalesLine.class))
                .list(app.getActiveShowIndex());
        if (asales == null) {
            p.m_lsales = new ArrayList<SalesLine>();
        } else {
   ...
   List<AdvSalesLine> Advasales = new StaticSentence(app.getSession(),
                ...
                "AND RECEIPTS.ADVANCEBOOKING = 'Y' AND TICKETS.TICKETTYPE=0 AND RECEIPTS.MONEYSHOW = ? " +
                "GROUP BY PRODUCTS.NAME"
                , SerializerWriteString.INSTANCE
                , new SerializerReadClass(PaymentsShowModel.AdvSalesLine.class))
                .list(app.getActiveShowIndex());
        if (Advasales == null) {
    ...
    List<GroupSalesLine> aGroupsales = new StaticSentence(app.getSession(),
                ...
                "GROUP BY PRODUCTS.NAME"
                , SerializerWriteString.INSTANCE
                , new SerializerReadClass(PaymentsShowModel.GroupSalesLine.class))
                .list(app.getActiveShowIndex());
        if (asales == null) {
   ...
   List<GroupAdvSalesLine> GroupAdvasales = new StaticSentence(app.getSession(),
                ...
                "GROUP BY PRODUCTS.NAME"
                , SerializerWriteString.INSTANCE
                , new SerializerReadClass(PaymentsShowModel.GroupAdvSalesLine.class))
                .list(app.getActiveShowIndex());
        if (Advasales == null) {
          ...
          List<RefSalesLine> Refasales = new StaticSentence(app.getSession(),
                ...
                "GROUP BY PRODUCTS.NAME"
                , SerializerWriteString.INSTANCE
                , new SerializerReadClass(PaymentsShowModel.RefSalesLine.class))
                .list(app.getActiveShowIndex());
        if (Refasales == null) {
            ...
            p.m_lRefsales = Refasales;
             ...
             List<DiscountSalesLine> Disasales = new StaticSentence(app.getSession(),
                ...
                "FROM RECEIPTS, TICKETLINES, PRODUCTS, TICKETS WHERE RECEIPTS.ID = TICKETS.ID AND TICKETS.ID = TICKETLINES.TICKET AND PRODUCTS.ID=TICKETLINES.PRODUCT  " +
                "AND TICKETS.DISCOUNT!=0   AND RECEIPTS.MONEYSHOW = ? " +
                "GROUP BY PRODUCTS.NAME"
                , SerializerWriteString.INSTANCE
                , new SerializerReadClass(PaymentsShowModel.DiscountSalesLine.class))
                .list(app.getActiveShowIndex());
        if (Disasales == null) {
        ...
        Object[] valTotalUnits = (Object []) new StaticSentence(app.getSession()
            , "SELECT SUM(TICKETLINES.UNITS) " +
              ...
              "WHERE TICKETS.ID = TICKETLINES.TICKET AND TICKETS.ID=RECEIPTS.ID AND RECEIPTS.MONEYSHOW = ? "
            , SerializerWriteString.INSTANCE
            , new SerializerReadBasic(new Datas[] {Datas.DOUBLE}))
            .find(app.getActiveShowIndex());
 ...
 public double getCancelFee() {
    ...
    public String printCancelFee() {
     ...
     public String printShowName() {
      ...
      public String printShowStarttime() {
        return m_StartTime;
    }
      public String printShowEndTime() {
        return m_EndTime;
    }
