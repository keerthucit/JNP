 ...
 * @author adrianromero
 */
public class PaymentsHandOverModel {
    ...
    private PaymentsHandOverModel() {
         sysDate = format.format(createdDate);
         sysTime = dt.format(createdDate);
    }    
    
    public static PaymentsHandOverModel emptyInstance() {
        
        PaymentsHandOverModel p = new PaymentsHandOverModel();
    ...
    public static PaymentsHandOverModel loadInstance(AppView app, String showName) throws BasicException {
        ...
        Object[] valtickets = (Object []) new StaticSentence(app.getSession()
            , "SELECT COUNT(*),  COALESCE(SUM(PAYMENTS.TOTAL),0) " +
              ...
              "WHERE PAYMENTS.RECEIPT = RECEIPTS.ID AND RECEIPTS.MONEYSHOW = ? AND PAYMENTS.PAYMENT='cash' AND RECEIPTS.ISHANDOVER='N' "
        ...
        Object[] valReftickets = (Object []) new StaticSentence(app.getSession()
            , "SELECT SUM(TICKETLINES.PRICE) " +
               "FROM RECEIPTS, TICKETLINES, PRODUCTS, TICKETS WHERE RECEIPTS.ID = TICKETS.ID AND TICKETS.ID = TICKETLINES.TICKET AND PRODUCTS.ID= TICKETLINES.PRODUCT " +
                "AND TICKETS.TICKETTYPE=1 AND RECEIPTS.MONEYSHOW = ? AND RECEIPTS.ISHANDOVER='N' "
            , SerializerWriteString.INSTANCE
        ...
        Object[] valCancelFee = (Object []) new StaticSentence(app.getSession()
            , "SELECT COALESCE(SUM(TICKETS.CANCELLATIONFEE),0) " +
              ...
              "WHERE TICKETS.ID = RECEIPTS.ID AND RECEIPTS.MONEYSHOW = ? AND RECEIPTS.ISHANDOVER='N' "
            , SerializerWriteString.INSTANCE
            , new SerializerReadBasic(new Datas[] {Datas.DOUBLE}))
            .find(app.getActiveShowIndex());
        ...
        List l = new StaticSentence(app.getSession()            
            ...
            , SerializerWriteString.INSTANCE
            , new SerializerReadClass(PaymentsHandOverModel.PaymentsLine.class)) //new SerializerReadBasic(new Datas[] {Datas.STRING, Datas.DOUBLE}))
        ...
        List<SalesLine> asales = new StaticSentence(app.getSession(),
               "SELECT PRODUCTS.NAME, SUM(TICKETLINES.UNITS), SUM(PRODUCTS.PRICESELL*TICKETLINES.UNITS) " +
                ...
                , SerializerWriteString.INSTANCE
                , new SerializerReadClass(PaymentsHandOverModel.SalesLine.class))
                .list(app.getActiveCashIndex());
        if (asales == null) {
            p.m_lsales = new ArrayList<SalesLine>();
        } else {
        ...
        List<AdvSalesLine> Advasales = new StaticSentence(app.getSession(),
              "SELECT PRODUCTS.NAME, SUM(TICKETLINES.UNITS), SUM(PRODUCTS.PRICESELL*TICKETLINES.UNITS) " +
                ...
                "GROUP BY PRODUCTS.NAME"
                , SerializerWriteString.INSTANCE
                , new SerializerReadClass(PaymentsHandOverModel.AdvSalesLine.class))
                .list(app.getActiveCashIndex());
        if (Advasales == null) {
    ...
    List<GroupSalesLine> aGroupsales = new StaticSentence(app.getSession(),
                ...
                "GROUP BY PRODUCTS.NAME"
                , SerializerWriteString.INSTANCE
                , new SerializerReadClass(PaymentsHandOverModel.GroupSalesLine.class))
                .list(app.getActiveCashIndex());
        if (asales == null) {
    ...
    List<GroupAdvSalesLine> GroupAdvasales = new StaticSentence(app.getSession(),
                ...
                "GROUP BY PRODUCTS.NAME"
                , SerializerWriteString.INSTANCE
                , new SerializerReadClass(PaymentsHandOverModel.GroupAdvSalesLine.class))
                .list(app.getActiveCashIndex());
        if (Advasales == null) {
          ...
          List<RefSalesLine> Refasales = new StaticSentence(app.getSession(),
                ...
                "GROUP BY PRODUCTS.NAME"
                , SerializerWriteString.INSTANCE
                , new SerializerReadClass(PaymentsHandOverModel.RefSalesLine.class))
                .list(app.getActiveCashIndex());
        if (Refasales == null) {
        ...
        if (valDiscounttickets == null) {
         ...
         List<DiscountSalesLine> Disasales = new StaticSentence(app.getSession(),
                ...
                "GROUP BY PRODUCTS.NAME"
                , SerializerWriteString.INSTANCE
                , new SerializerReadClass(PaymentsHandOverModel.DiscountSalesLine.class))
                .list(app.getActiveCashIndex());
        if (Disasales == null) {
